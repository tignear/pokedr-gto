...
use pokedr_core::{
    ...
    dense_cfr::gpu::{
        ...
        GpuShowdownTask, GpuTerminalAction,
    },
...
fn linearize_gpu_public_tree(
    ...
    let mut showdown_values = Vec::new();
    ...
    for (node_index, node) in tree.nodes().iter().enumerate() {
        ...
        match &node.kind {
            ...
            PublicNodeKind::Terminal {
                ...
                child_cards.extend(std::iter::repeat_n(52, node.children.len()));
                let (terminal_kind, showdown_offset) = match kind {
                    TerminalKind::Fold => (fold_terminal_code(tree, node_index)?, 0),
                    TerminalKind::Showdown => {
                        let key = board.deck_mask();
                        let offset = if let Some(offset) = showdown_offsets.get(&key) {
                            *offset
                        } else {
                            if !matrix_cache.borrow().contains_key(&key) {
                                ...
                                let matrix =
                                    backend.showdown_matrix(&combos, &final_boards).ok()?;
                                matrix_cache.borrow_mut().insert(key, matrix);
                            }
                            let offset = showdown_values.len() as u32;
                            showdown_values.extend(matrix_cache.borrow().get(&key)?.iter());
                            showdown_offsets.insert(key, offset);
                            offset
                        };
                        (2, offset)
                    }
    ...
    if showdown_values.is_empty() {
        showdown_values.push(0.5);
    }
