...
pub(super) const PUBLIC_TREE_LAYER_COMPACT_FUSED_UPDATE_SHADER: &str = r#"
struct Combo {
    cards: array<u32, 2>,
};
struct Edge {
...
fn strategy_at_fused(slot: u32, child_count: u32, normalizer: f32) -> f32 {
    if normalizer > 0.0 {
        return positive_fused(effective_regret_fused(slot)) / normalizer;
    }
    return 1.0 / f32(max(child_count, 1u));
}
...
fn fused_update_tile(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    for (var action = 0u; action < node.child_count; action = action + 1u) {
        normalizer = normalizer + positive_fused(effective_regret_fused(compact_action_slot(node, combo, action)));
    }

    var q_values: array<f32, 8>;
    var node_value = 0.0;
    let reach_weight = node.public_chance_reach * counterfactual_weight;
    for (var group_edge = 0u; group_edge < group.edge_count; group_edge = group_edge + 1u) {
        let edge = edges[group.first_edge + group_edge];
        let child_offset = edge.child * params.combo_count + combo;
        let raw_action_value = select(
        ...
        let q = raw_action_value / reach_weight;
        q_values[edge.action] = q;
        let strategy = strategy_at_fused(compact_action_slot(node, combo, edge.action), node.child_count, normalizer);
        node_value = node_value + strategy * q;
    }

    let average_discount = average_strategy_discount_fused();
    var strategy_weight = node.public_chance_reach * own_reach * f32(params.iteration);
    if params.variant == 2u || params.variant == 3u || params.variant == 4u || params.variant == 5u {
        strategy_weight = node.public_chance_reach * own_reach;
    }
    if params.avg_delay != 0u || bitcast<f32>(params.avg_power_bits) != 0.0 {
        strategy_weight = node.public_chance_reach * own_reach * average_strategy_weight_multiplier_fused();
    }

    for (var action = 0u; action < node.child_count; action = action + 1u) {
        let slot = compact_action_slot(node, combo, action);
        let strategy = strategy_at_fused(slot, node.child_count, normalizer);
        let regret = (q_values[action] - node_value) * reach_weight;
        var updated = regrets[slot] * regret_discount_fused(regrets[slot]) + regret;
        if params.variant == 0u || params.variant == 2u || params.variant == 3u || params.variant == 4u {
            updated = max(updated, 0.0);
        }
        regrets[slot] = updated;
        if params.variant == 3u {
            prediction[slot] = regret;
        }
        strategy_sum[slot] = strategy_sum[slot] * average_discount + strategy_weight * strategy;
    }
}
"#;
...
fn private_infoset_action_offset(node: TreeNode, private_combo: u32) -> u32 {
    let public_base = params.node_count;
    let private_infoset = (node.public_infoset - public_base) * params.combo_count + private_combo;
    return private_infoset * params.max_actions;
}

fn strategy_normalizer(offset: u32, child_count: u32) -> f32 {
    var normalizer = 0.0;
...
fn reach_edge_tile(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    if node.kind == 0u {
        ...
        let action_offset = private_infoset_action_offset(node, combo);
        let normalizer = strategy_normalizer(action_offset, node.child_count);
        for (var group_edge = 0u; group_edge < group.edge_count; group_edge = group_edge + 1u) {
            let edge = edges[group.first_edge + group_edge];
            let child_offset = edge.child * params.combo_count + combo;
...
pub(super) const PUBLIC_TREE_LAYER_COMPACT_BACKUP_SHADER: &str = r#"
struct TreeNode {
    kind: u32,
    acting_player: u32,
    public_infoset: u32,
...
@group(0) @binding(11) var<storage, read> prediction: array<f32>;
@group(0) @binding(12) var<uniform> params: Params;
@group(0) @binding(13) var<storage, read> public_action_offsets: array<u32>;
